    ...
    <div class="flex flex-col lg:flex-row px-4 lg:px-32 space-x-0 lg:space-x-4">
      <div class="w-full lg:w-4/12 my-6 py-2">
        <div class="flex flex-col rounded-lg bg-white shadow justify-center items-center py-14">
          <div class="flex justify-center items-center rounded-full bg-primary p-2 w-32 h-32">
      ...
      <div class="w-full lg:w-8/12 pt-6">
        <div class="flex flex-row space-x-10 justify-between pb-3 items-start md:items-center">
          ...
          <router-link to="/complaint/add">
            <button class="text-sm text-white rounded-lg bg-primary px-4 py-2 w-30">Add Complaint</button>
          </router-link>
        </div>
        <MaterialTable :tableData="tableData" :tableHeader="tableHeader" @edit="editComplaint"
...
const getComplaints = () => {
  ...
  action.then((res) => {
    ...
    if (userType === 'A') {
      res.map((v, index) => {
        const { complaint_no, complaint_date, comp_status, po_number, customer_id } = v;
        tableData.value.push({
          complaint_no: 'CCR-' + complaint_no,
          complaint_date,
          po_number,
          "customer_name": customersList?.value?.find(cs => cs.key === customer_id).value,
          "project_status": comp_status,
          complaint_status: complaint_no,
        });
      });
    } else {
      res.map((v, index) => {
...
const editComplaint = (payload) => {
  router.push("complaint/" + payload.complaint_no.split('CCR-')[1]);
};

const deleteComplaint = (id) => {
  if (window.confirm("Do you really want to delete?")) {
    cStore.$deleteComplaint(id.split('CCR-')[1]).then(async (res) => {
      notify(
        {
          group: "success",
          title: "Success",
